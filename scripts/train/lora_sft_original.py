...
モデルをローカルから読む
"""
MODEL_DIR = "./finetuning/model/gemma-2-2b"
...
training_args = transformers.TrainingArguments(
    ...
    gradient_accumulation_steps=4,   # 実効バッチ=4
    warmup_steps=2,
    max_steps=20,                    # 小さく試し、動作確認後に増やす
    learning_rate=2e-4,
    logging_steps=1,
    save_steps=20,
    save_total_limit=1,
    fp16=False,                      # 【変更点】CPUではFalse
    bf16=False,                      # 【変更点】CPUではFalse
    optim="adamw_torch",             # 【変更点】GPU向け paged_adamw_8bit をやめる
    report_to="none"
)
...
trainer = SFTTrainer(
    ...
    # skouai/Kansai-Obachan は train スプリットを持っていないため、test スプリットを使う
    # train_dataset=data["test"],
    formatting_func=formatting_func,  # formatting_func で素のテキストを渡す
    args=training_args,
    tokenizer=tokenizer,              # tokenizerを渡すと内部で適切にトークナイズされる
    max_seq_length=1024,              # 長すぎるとCPUメモリが厳しいので上限を明示
)
